...
output.forest <- randomForest(NDVI ~ vec_mean_temp + vec_mean_precip + Green, 
...
output.forest <- randomForest(as.factor(Landcover) ~ vec_mean_temp + vec_mean_precip + NDVI, 
                              data = matrix_full_train, ntree=100)
...
# W_Ecosystem = variable la plus importante dans le modèle 
  # --> type d'habitat jouent un rôle prépondérant dans la détermination de la distribution des espèces
# autres variables ayant un fort impact sur la distribution des espèces: elevation et NDVI
  # NDVI = couverture végétale est aussi une variable prépondérante dans la distribution des espèces